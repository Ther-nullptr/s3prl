...
class UpstreamPretrainExpert(nn.Module):
    ...
    def __init__(self,
        ...
        if type(upstream_config) == str:
            ...
            logger.info("[UpstreamPretrainExpert] - Using upstream config")
...
class DistillerForPretrain(nn.Module):
    ...
    def __init__(self, config: DistillerConfig, teacher_config: edict):
        ...
        if teacher_config.model.find("wav2vec2") >= 0:
            logger.info(
                "[DistillerForPretrain] - use wav2vec2 as teacher model")
            ...
            logger.info(
                "[DistillerForPretrain] - use data2vec as teacher model")
        ...
        logger.info(
            "[DistillerForPretrain] - Using {} as teacher with {} layers".
        ...
        if self.cosine_loss > 0:
            logger.info(
                "[DistillerForPretrain] - Enabled cosine similarity loss.")
        ...
        # kmeans
        self.kmeans_path = config.kmeans_path
        self.kmeans_layer = config.kmeans_layer
        self.kmeans_model = ApplyKmeans(config.kmeans_path)
    ...
    def forward(
        ...
        if self.config.get_hidden == False:
            feat, feat_final, pred, pad_mask, student_embeddings = self.distiller(
                wave_input, pad_mask)  #! distiller model(the small one)
        else:
            feat, feat_final, pred, pad_mask, student_hiddens, student_attns = self.distiller(
                wave_input, pad_mask, get_hidden=True)
        ...
        with torch.no_grad():
            ...
            with torch.cuda.amp.autocast(False):
                ...
                teacher_logits = teacher_hiddens["hidden_states"][11 - self.kmeans_layer]  # B x T x C
                teacher_logits = teacher_logits.reshape(teacher_logits_shape[0] * teacher_logits_shape[1], teacher_logits_shape[2])
                teacher_embeddings = self.kmeans_model(teacher_logits) # BT x 500
        ...
        (
            total_loss, rec_loss, rec_layer_loss, feat_pen, sim_loss,
            sim_layer_loss, emb_loss
        ) = self.compute_loss(
            feat, pred, teacher_hiddens, teacher_embeddings,
            student_embeddings, return_other
    ...
    def compute_loss(self,
        ...
        if self.embedding_loss > 0:
            embedding_size = student_embeddings.shape
            with torch.no_grad():
                teacher_embedding_logits = teacher_embeddings / self.temperature

            student_embedding_logits = student_embeddings.view(
                embedding_size[0] * embedding_size[1],
                embedding_size[2])  # [b*t,500]
            if self.use_temperature:
                student_embedding_logits = student_embedding_logits / self.temperature
            teacher_prob_log = F.log_softmax(teacher_embedding_logits,
                                                dim=-1)
            student_prob_log = F.log_softmax(student_embedding_logits,
                                                dim=-1)
            ...
            if (self.steps % 200 == 0 and self.enable_decode):
                ...
                with torch.no_grad():
                    teacher_prob_log_t = teacher_prob_log.view(
                        embedding_size[0], embedding_size[1],
                        -1).float().contiguous().cpu().unsqueeze(
                            0)  # 1 x B x T x kinds
                    student_prob_log_t = student_prob_log.view(
                        embedding_size[0], embedding_size[1],
                        -1).float().contiguous().cpu().unsqueeze(
                            0)  # 1 x B x T x kinds
                    for (teacher_lp,
                         student_lp) in zip(teacher_prob_log_t[0:3],
                                            student_prob_log_t[0:3]):
                        ...
                        if (teacher_decoded is not None
                                and student_decoded is not None
                                and "tokens" in teacher_decoded
                                and "tokens" in student_decoded):
                            ...
                            w_errs += editdistance.eval(
                                teacher_string.split(" "),
                                student_string.split(" "))
        ...
        total_loss = (rec_loss * self.rec_loss +
