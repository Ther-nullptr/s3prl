...
class DistillerModel(nn.Module):
    ...
    def forward(self, wave, pad_mask, task_id=None, get_hidden=False, no_pred=False): #! to get the output of distill model
        ...
        masked_embeddings, no_masked_embeddings, masked_indices, no_masked_indices = self.get_embeddings(hidden) # B x T x E
    ...
    def get_embeddings(self, x, mask_indices, pad_mask):
        ...
        no_masked_indices = torch.logical_and(~pad_mask, ~mask_indices)
